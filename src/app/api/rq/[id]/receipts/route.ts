  ...
  try {
    const rq = await prisma.rQ.findUnique({
      ...
      select: {
        ...
        status: true,
        items: {
          select: { id: true, name: true, qty: true, uom: true, spec: true },
          orderBy: { id: 'asc' },
        },
        receipts: {
          ...
          select: {
            ...
            items: {
              select: {
                ...
                qtyReceived: true,
                notes: true,
                rqItem: { select: { name: true, uom: true } },
              },
            },
          },
        },
    ...
    return NextResponse.json({
      rq: { code: rq.code, title: rq.title, status: rq.status },
      rqItems: rq.items,
      receipts: rq.receipts,
    })
  } catch (err) {
    console.error(err)
    return NextResponse.json({ rq: null, rqItems: [], receipts: [] })
  }
})
